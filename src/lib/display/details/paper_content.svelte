<script lang="ts">
	...
	const no_access_text = "This information is not available because we couldn't access the full text of the paper";
	...
	const paper_type_parts = $derived.by(() =>
	{
		...
		if (paper.type !== MissingReason.NotApplicable)
		{
		...
		else if (paper.on === MissingReason.NotSpecified)
		{
			result.push({
				...
				description: (
					paper.review ?
					`This ${cards.TO_TEXT[paper.review.type].toLowerCase()} does not specify TODO` :
					'The subjects of the study are not explicitly specified in the paper'
				),
				is_card: true,
			});
		}
	...
	const sample_size = $derived.by(() =>
	{
		...
		else if (paper.sample_size === MissingReason.NotSpecified)
		{
			return {
				...
				color: COLORS[Color.Gray].default,
				shadow: color_to_shadow(COLORS[Color.Gray].default),
				description: 'The paper does not specify the number of participants included in the study',
				is_card: true,
			}
		}
	...
	const citations = $derived.by(() =>
	{
		if (paper.citations.count === MissingReason.NotSpecified)
		{
			return {
				...
				color: COLORS[Color.Gray].default,
				shadow: color_to_shadow(COLORS[Color.Gray].default),
				description: 'The number of times the paper has been cited in other papers in unknown',
				is_card: true,
			}
		}

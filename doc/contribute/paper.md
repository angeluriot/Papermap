...
* The options are:
	...
	* `(Diverse types)`: the paper is a literature review that includes studies of various types (only for literature reviews)
...
* If the paper is a literature review, use the most common type of study in the reviewed papers, or `(Diverse types)` if the paper includes studies of various types
...
* **Impact** is a subjective assessment of the information's effect on the paper's credibility or its impact on the map's question:
	...
	* `Negative` is for information that may reduce the credibility of the paper or its impact on the map's question, for example if the product studied is no longer in use today
